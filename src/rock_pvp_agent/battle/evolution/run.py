...
def _opponent_player(spec, side: str, seed: int, *, settings):
    ...
    from rock_pvp_agent.battle.evolution.styles import StylePlayer
    from rock_pvp_agent.battle.player import PlaybookPlayer
...
def _aggregate_report(playbook, instances, *, seeds, team_size, lives, llm, settings) -> dict:
    ...
    """
    from rock_pvp_agent.battle.evolution.bench import paired_eval, wilson_ci
...
def _tr_minibatch(team_size: int, *, n_seeds: int, seed_base: int = 100) -> list:
    ...
    from rock_pvp_agent.battle.evolution.bench import Instance, tr_spirit_pool
...
def run_steps(*, n: int, seed: int = 7, out_dir: str = "artifacts",
    ...
    **⑧有界编辑** → **⑨廉价门**（同一 minibatch，未改进 → 写入 rejected buffer，本 step 结束）
    ...
    from rock_pvp_agent.battle.evolution.league import (
...
def run_epochs(*, n: int = 8, seed: int = 7, out_dir: str = "artifacts",
    ...
    """
    from rock_pvp_agent.battle.evolution.bench import build_instances
    from rock_pvp_agent.battle.evolution.editor import diff_rules, slow_update
    ...
    from rock_pvp_agent.battle.evolution.pool import PlaybookPool
    from rock_pvp_agent.battle.evolution.valuefn import train_value_fn
