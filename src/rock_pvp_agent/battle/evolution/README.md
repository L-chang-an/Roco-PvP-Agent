...
```bash
python -m rock_pvp_agent evolve eval --bench d_sel       # 配对评测
python -m rock_pvp_agent evolve reflect --traj runs/x.json   # 轨迹分析
python -m rock_pvp_agent evolve credit --traj runs/x.json    # 信度分配
python -m rock_pvp_agent evolve step                         # 单步进化
python -m rock_pvp_agent evolve health --memory <dir>        # 记忆健康度
```
