...
> **状态**：**Plan v1 定稿**（2026-08-26）。启动前 8 条决策已全部拍板（见 §十五 决策记录）；
> 核心闭环 R0–R5 待逐里程碑实施，每个 Gate 负责人确认"通过"才进下一步。
>
...
- **可审计但不追求攻略级可读（负责人 2026-08-26 拍板）**：模块化 + 单规则条目 +
  `edit_apply_report` 全程可追溯 + `[PROTECTED]` 保护区——负责人能看懂每条规则在说什么、
  能回溯每版变化；但**不把"读起来像攻略"当进化约束**（GEPA 对抗实验：可读性 ≠ 有效性，
  强求可读反而诱导"看起来合理实则有害"的文本）。
...
④ Rollout     用 c 对 o 跑 B=4 场配对对局（双向先后手、固定种子、固定阵容族；
              默认 B=4 对应 10M token/epoch 预算，预算宽裕可升到 8，见 §十/§十五决策2）
...
### 7.1 单池模式（**已启用**，负责人 2026-08-26 拍板起步）
...
### 7.2 独立双代理模式（可选扩展，成本约翻倍；**2026-08-26 拍板：未启用**，仅作扩展备用）
...
| `D_test` | 隐藏集 | 人工反套路场景 + 未公开阵容 → 只用于汇报 | 不进任何优化输入；**逻辑隔离**（独立目录 + 独立命名 + 单入口汇报脚本 + `data_digest` 校验；负责人 2026-08-26 拍板不上加密）；每季度换血 30% |
...
| 两级门禁的廉价门 | 绝大多数候选死在 4 场 minibatch 上，不付全量 `D_sel` | R3 起 |
| 反事实回放前缀复用 | `CounterfactualCache` 按 `(trajectory_id, turn_no)` 共享 | R2 起 |
| SPRT 早停 | 强候选早停晋级、弱候选早停拒绝 | R4 起 |
| 优化器与目标可不同档 | **已拍板允许**（§十五决策 1）。SkillOpt：强优化器收益更大；同档仍回收 56–74% 增益 | R3 起 |

**拍板预算（负责人 2026-08-26）：10M token/epoch。** 估算公式与默认参数组合（详见 §十五决策 2）：

```
Token_epoch ≈ E × (R×T×C_r + M×C_ref + G_cheap×T×C_g + P_full×I×g×T×C_g)
默认：E=8, R=4, T=20(3v3/2命), C_r=5k, M=5, C_ref=9k, G_cheap=2, C_g=2.5k,
      P_full=0.25, I=8, g=4
→ 每 step ≈ 0.95M，每 epoch ≈ 7.6M token（留 ~24% 余量到 10M 上限）
```

反事实回放（代理尾策略）是**纯引擎计算、零 token**，不算进预算。预算命门是
`R×T×C_r`（rollout）与 `P_full×I×g×T×C_g`（全量门）。跑通第一个 epoch 后把实际
token 记账（`edit_apply_report` 顺手记）回填校准公式。

按 step 记账：单 step = `B=4` 场 rollout（含先后手）+ 关键回合挖掘（含回放，零 token）+
反思调用 + 门禁评测。
...
```python
def two_phase_search(store, query: MemoryQuery, *, delta=0.5, k1=10, lam=0.5, k2=3) -> list[MemoryEntry]:
...
| `memory_embedder` | `"keyword"` | `keyword`(兜底，负责人拍板默认) / 真实嵌入(可选) |
...
| `reflection_model` | 独立档位（可更便宜或更强） | 反思/编辑 LLM 档位（负责人拍板允许不同档；默认建议更便宜，预算宽裕可用更强） |
| `battle_rollout_b` | `4` | 单 step rollout 场数（配对对局；10M 预算下默认 4，宽裕可升 8） |
| `budget_per_epoch` | `10_000_000` | 单 epoch token 预算上限（记账用；超限暂停并告警） |
| `v_tier` | `"a0"` | 价值函数档位（a0 启发式 / a1 训练模型） |

---

## 十五、决策记录（负责人已拍板，2026-08-26）

> 启动前的 8 条待确认项已逐条拍板。每条附：问题解释 → 拍板 → 理由 → 对正文的具体影响。

| # | 决策点 | 拍板 | 问题解释 / 理由 | 对方案的影响 |
|---|---|---|---|---|
| 1 | **优化器模型档位** | **允许不同档**（可更便宜或更强） | 方案里有两类 LLM：对战的（被优化，成本大头）与反思/编辑的（离线批处理，训练期一次性）。"档位"= 模型强弱/价格档。SkillOpt 实测：强优化器收益更大且不增部署成本；同档优化器仍回收 56–74% 增益，不会失效 | `reflection_model` 独立可配；§十"对手用便宜档位"同样成立 |
| 2 | **预算上限** | **10M token/epoch 最小预算** | 决定 `B`（rollout 场数）、`M`（回放场次）、`D_sel` 实例数。估算公式与默认参数组合见下方"决策 2 落地" | 默认参数组合调整为 `B=4` 等（见 §十）；预算宽裕再升 `B` |
| 3 | **`battle_act` 加 `prediction` 参数** | **接受**（可选参数，向后兼容） | 加一个可选字段让 LLM 每回合顺带预期结果，离线与结算比对 → "校准偏差"信号（区分运气差 vs 算错了）。可选+缺省保证旧调用合法、`replay_record` 忽略未知键；成本每局 ~1–2k token | §6.1/§6.2 校准偏差启用；每回合每方 ~20–50 token 输出成本 |
| 4 | **D_test 物理隔离** | **逻辑隔离即可**（不上加密） | 单人本地项目，风险是"自己写脚本手滑把 d_test 当 d_sel"，不是外部访问。加密防的是想象中的敌人且每次汇报要解密 | §八：独立目录 + 独立命名 + 单入口汇报脚本 + `data_digest` 校验；上 CI 再升级权限 |
| 5 | **对局规模基准** | **3v3 / 2 命** | 规模线性影响每局时长与 token 成本；3v3 每局 ~20 回合，适合在预算内快速迭代 | R0 评测与 rollout 默认规模；`T≈20` 进预算公式 |
| 6 | **Playbook 人类可读** | **无需攻略级可读，但必须可审计** | Playbook 是注入 LLM 的自然语言文本，字符层面永远可读；要拍板的是"要不要把它当攻略产出"。攻略级可读会牺牲优化空间（SkillOpt 学到的技能 379–1,995 token、收益来自 1–4 次编辑），且 GEPA 对抗实验证明可读性 ≠ 有效性 | §3.1：模块化 + 单规则条目 + `edit_apply_report` 全程可追溯 + `[PROTECTED]` 保护区 |
| 7 | **双代理模式（§7.2）** | **单池模式起步**，双代理留作扩展 | 单池模式 = 一个 Playbook 池 + 共享记忆按 side 更新 Q，最省成本且最贴论文；双代理成本约翻倍 | §7.1 为唯一启用形态；§7.2 标注"未启用，仅作扩展备用" |
| 8 | **`memory_embedder`** | **默认零依赖 `keyword` 兜底** | KeywordEmbedder = situation_key 硬过滤 + 结构化标签 + TF-IDF，纯标准库、零网络、确定性。本游戏局面高度结构化，硬过滤已占第一优先级；MemRL 增益来自 z-score + Q 重排，不依赖嵌入实现 | §6.3/§十四：`memory_embedder="keyword"` 默认；`Embedder` Protocol 可插拔缝保留 |

**决策 2 的落地——10M token/epoch 下的默认参数组合**：

```
Token_epoch ≈ E × ( R×T×C_r            # ① rollout（预算命门）
                  + M×C_ref            # ② 反思/编辑
                  + G_cheap×T×C_g      # ③ 廉价门
                  + P_full×I×g×T×C_g ) # ④ 全量门（按概率摊销）
默认：E=8（每 epoch step 数）· R=4（每 step rollout 配对场数）· T=20（3v3/2命 回合数）
      C_r=5k（一回合双方调用 token）· M=5（每 step 反思+编辑调用）· C_ref=9k（单次反思）
      G_cheap=2（廉价门场数）· C_g=2.5k（门禁一回合，对手启发式→单边前沿）
      P_full=0.25（每 step 触发全量门概率）· I=8（全量门实例数）· g=4（每实例场数）
→ 每 step ≈ 0.95M，每 epoch ≈ 7.6M token（留 ~24% 余量到 10M 上限）
```

反事实回放（代理尾策略）是**纯引擎计算、零 token**，不算进预算。降本旋钮优先级：
① 门禁评测对手用便宜档/启发式（单边化）；② 提示缓存（Playbook+图鉴静态前缀省 40–60%）；
③ 廉价门压低 `P_full`；④ 3v3 而非 6v6（`T` 线性乘进公式）。
