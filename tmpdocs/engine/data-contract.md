...
- **防御冷却（2026-08-30 修订机制，取代早期「TURN_END 递减」口径）**：
  使用防御技能 → 该精灵**所有防御类技能** cd=1；**冷却衰减只看精灵是否完整经历
  「冷却回合」的两个节点**（**两个节点缺一不可**、**无需完整在场持续一回合**）：
  - 「冷却回合开始」节点 = 释放防御后的下一回合开始时，精灵在场；
  - 「冷却回合结束」节点 = 经历回合结束 / 换下离场 / 阵亡等回合边界；
  - 两个节点都经历 → cd −1（实现：回合入口递减 =「开始」节点，开始在场者必经历结束）；
  - **只经历「结束」节点不衰减**：释放防御当回合即离场 → 未经历「开始」节点 →
    之后即使阵亡补位上场，也只经历了「结束」节点，冷却不递减（回场仍被禁）；
...
- **阵亡清层**：非永久 buff 与冻结层清除；永久层（萌化等）保留——复活特性复活后
  仍带永久 buff（除冻结）；
- **冻结语义（2026-08-30 修正）**：冻结本身**不含能耗副作用**——语义只是抬高血量
  斩杀线（血量低于冻结层数×5% 力竭）；任何「通过冻结加能耗」都靠显式 `energy_cost`
  debuff 实现：捉迷藏/抓到你了（每次给敌方施加冻结 → 敌方 +1 层 energy_cost）、
  霜天（施加「等于冻结层数」的 energy_cost 层）。
...
| **DOT 持久性（2026-08-30 拍板）**：中毒/灼烧/寄生/引电非永久（离场清除）；萌化/冻结永久（离场保留）；冰免疫冻结 + 特性级「免疫冻结」标记；阵亡清层（非永久+冻结清除、永久保留）；**冻结不含能耗副作用**（加能耗靠显式 energy_cost debuff：捉迷藏/霜天） | statuses.py / reducer.py / engine.py / primitives.py |
