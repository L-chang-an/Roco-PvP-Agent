...
| **DOT 持久性（2026-08-30 拍板）**：中毒/灼烧/寄生/引电非永久（离场清除）；萌化/冻结永久（离场保留，冻结阵亡清除）；冰免疫冻结 + 特性级「免疫冻结」；阵亡清层（非永久+冻结清除、永久层保留——复活特性）；冻结固有副作用每层全技能能耗+1 | statuses.py / reducer.py / engine.py / primitives.py / tests/test_environment_status_persistence.py |
