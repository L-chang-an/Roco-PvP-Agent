...
def mirror_pair() -> tuple[list[dict], list[dict]]:
    """双方同规格同速 → 平手硬币必然有机会触发。"""
    picks = [pick("迪莫", ["抓挠1", "加物攻"]), pick("小火猴", ["抓挠", "撞击1"]),
             pick("水蓝蓝", ["撞击", "加魔攻"])]
    return team(*picks), team(*picks)


def duel() -> tuple[list[dict], list[dict]]:
    """1v1 平衡对局（配 RULES_1V1）：a 先手（速 100>90），三系技能齐全。"""
    a = [spec("甲", 300, 100, 100, 100, 100, 100, ["抓挠1", "加物攻", "防御"])]
    b = [spec("乙", 300, 100, 100, 100, 100, 90, ["撞击1", "加魔攻", "防御"])]
    return a, b
...
def strong_weak() -> tuple[list[dict], list[dict]]:
    ...
    b = [spec("弱靶", 50, 20, 20, 20, 20, 50, ["撞击"])]
    return a, b
