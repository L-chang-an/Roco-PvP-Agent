...
def test_team_size_bounds() -> None:
    ...
    assert validate_team_size(4) is None
    assert validate_team_size(MAX_TEAM_SIZE) is None
    assert "3–6" in validate_team_size(MIN_TEAM_SIZE - 1)
    assert "3–6" in validate_team_size(MAX_TEAM_SIZE + 1)
...
def test_lives_bounds() -> None:
    assert validate_lives(4, 1) is None
    assert validate_lives(4, 3) is None
    assert "1–3" in validate_lives(4, 0)          # 低于 1
    assert "1–3" in validate_lives(4, 4)          # 等于 team_size（必须小于）
    assert validate_lives(3, 2) is None
    assert "1–2" in validate_lives(3, 3)


def test_lives_non_int() -> None:
    assert "整数" in validate_lives(4, 2.5)
    assert "整数" in validate_lives(4, True)


def test_lives_requires_valid_team_size_first() -> None:
    assert "3–6" in validate_lives(7, 1)          # 先报 team_size，再谈 lives


# ── build_battle_rules（管理员接口）──
def test_build_rules_valid() -> None:
    rules = build_battle_rules(team_size=4, lives=3)
    assert rules.team_size == 4 and rules.lives == 3 and rules.skill_slots == 4
...
def test_build_rules_invalid_raises() -> None:
    ...
    with pytest.raises(ValueError, match="1–3"):
        build_battle_rules(team_size=4, lives=4)
