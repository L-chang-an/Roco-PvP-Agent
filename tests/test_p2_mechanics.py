...
def test_combo_flat_buff_adds_hits() -> None:
    ...
    a, b = _mk("甲", ["普通"], ["热身运动", "乱打"]), _mk("乙", ["普通"], ["抓挠"])
    s = _battle(a, b)
...
def test_combo_pct_buff_doubles_hits() -> None:
    ...
    a, b = _mk("甲", ["普通"], ["暴风眼", "乱打"]), _mk("乙", ["普通"], ["抓挠"])
    s = _battle(a, b)
...
def test_combo_buff_only_boosts_combo_skills() -> None:
    """连击数buff 只加成「带有连击描述」的技能：抓挠（无连击）不被加成。"""
    a, b = _mk("甲", ["普通"], ["热身运动", "抓挠"]), _mk("乙", ["普通"], ["抓挠"])
    s = _battle(a, b)
    _turn(s, skill_action(0))                       # 热身运动 +3
    events = _turn(s, skill_action(1))              # 抓挠（combo_eligible=False）
...
def test_priority_plus_beats_speed() -> None:
    """先手+1（俯冲）虽慢仍先于快但普通优先级（抓挠）的对手出手。"""
    a, b = _mk("甲", ["普通"], ["俯冲"]), _mk("乙", ["普通"], ["抓挠"])
    s = _battle(a, b)
...
def test_steal_energy() -> None:
    ...
    assert st["gained"] == 3
    assert b.energy == 3                              # 5-3(偷)+1(抓挠回能)
...
def test_bench_energy_status() -> None:
    ...
    a0, a1 = _mk("甲0", ["普通"], ["富养化"]), _mk("甲1", ["普通"], ["抓挠"])
    b0, b1 = _mk("乙0", ["普通"], ["抓挠"]), _mk("乙1", ["普通"], ["抓挠"])
...
def test_bench_energy_attack() -> None:
    """养分回流：造成魔伤 + 场下每只回1能量。"""
    rules = replace(DEFAULT_RULES, team_size=2)
    a0, a1 = _mk("甲0", ["普通"], ["养分回流"]), _mk("甲1", ["普通"], ["抓挠"])
    b0, b1 = _mk("乙0", ["普通"], ["抓挠"]), _mk("乙1", ["普通"], ["抓挠"])
    s = BattleState(side_a=SideState(units=[a0, a1], lives=2),
                    side_b=SideState(units=[b0, b1], lives=2),
                    rng=BattleRng(7), rules=rules)
...
def test_per_hit_foe_energy_cost() -> None:
    ...
    a, b = _mk("甲", ["普通"], ["冰捆缚"]), _mk("乙", ["普通"], ["抓挠"])
    s = _battle(a, b)
    _turn(s, skill_action(0))
    assert skill_energy_cost(None, "a", b, 2) == 4  # 抓挠 base 2 → 4
...
def test_sanlianpo_eats_combo_buff() -> None:
    ...
    a, b = _mk("甲", ["普通"], ["热身运动", "三连破"]), _mk("乙", ["普通"], ["抓挠"])
    s = _battle(a, b)
...
def test_wuqi_huanrao_energy_from_foe_cost() -> None:
    ...
    a, b = _mk("甲", ["普通"], ["雾气环绕"]), _mk("乙", ["普通"], ["抓挠", "能量炮", "地震"])
    s = _battle(a, b)
    a.energy = 3
    events = _turn(s, skill_action(0))
    foe_cost = sum(sk.energy_cost for sk in b.skills)     # 0+3+10 = 13（抓挠能耗 0）
    eg = [e for e in events if e["type"] == "energy_gain"][0]
    assert eg["gained"] == foe_cost // 2                   # 6
    assert a.energy == 3 - 1 + 6                           # 先付能耗 1 + 回 6 = 8
